		...
		private void AnalyzeBox(decimal hmx, decimal hy, decimal hmy, decimal hz, decimal hmz, decimal dim1, decimal dim2, decimal dim3)
		{
			if (dim1 <= hmx && dim2 <= hmy && dim3 <= hmz)
			{
				if (dim2 <= hy)
				{
					...
					else if (hy - dim2 == bfy && hmx - dim1 == bfx && m.Abs(hz - dim3) < bfz)
					{
						boxx = dim1;
						boxy = dim2;
						boxz = dim3;
				...
				else
				{
					...
					else if (dim2 - hy == bbfy && hmx - dim1 == bbfx && m.Abs(hz - dim3) < bbfz)
					{
						bboxx = dim1;
						bboxy = dim2;
						bboxz = dim3;
				...
				for (y = 1; y <= 3; y++)
				{
					...
					if ((exdim <= thickness) && (((dimen2 <= px) && (dimen3 <= pz)) || ((dimen3 <= px) && (dimen2 <= pz))))
					{
						for (z = 1; z <= itemsToPackCount; z++)
						{
							...
							{
								dimdif = m.Abs(exdim - itemsToPack[z].Dim1);

								if (m.Abs(exdim - itemsToPack[z].Dim2) < dimdif)
								{
									dimdif = m.Abs(exdim - itemsToPack[z].Dim2);
								}

								if (m.Abs(exdim - itemsToPack[z].Dim3) < dimdif)
								{
									dimdif = m.Abs(exdim - itemsToPack[z].Dim3);
								}
